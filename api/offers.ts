...
router.post('/create', authorizeOnlyCompany, async (req, res) => {
    ...
    try {
        const profile = await ProfileCompany.findOne({ userUsername: companyUsername})    
        if (!profile) {
            return res.status(404).json({ error: `profile '${companyUsername}' not found` })
        }
...
router.get('/list', async (req, res) => {
    ...
    try {
        const offers = await Offer.find({ startingDate: { $gt: new Date() } }, 'company.name company.logo name startingDate duration location description skills')
        let filteredOffers;
        switch (filter) {
            case 'gpa':
...
router.get('/:offerId', async (req, res) => {
    ...
    try {
        const offer = await Offer.findOne({ _id: id }, 'company.name company.logo description responsibilities qualifications -_id')
